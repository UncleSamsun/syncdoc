...
MVP가 구현할 계약 23개 전부다. 이 표에 없는 엔드포인트는 구현 대상이 아니다. 조건이 많은 계약은 아래에 같은 ID의 절을 두고, 표의 `응답·조건` 칸에서 그 절을 가리킨다.
...
| API-002 | `GET /auth/github/callback` | REQ-001, REQ-007 | code, state | state 일치·만료 검사와 허용 목록 확인 후 세션 회전과 302. 실패 시 세션을 발급하지 않는다 |
