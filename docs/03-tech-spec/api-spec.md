...
**입력:** 본문에 `fullName`을 받는다. 지금 연결된 저장소의 전체이름과 **정확히 같아야** 실행한다. 되돌릴 수 없는 일이라 한 번 더 확인받는 것이며, 다르면 422 `INVALID_REQUEST`다.

**접근 조건:** 연결자 또는 서비스 관리자만 부를 수 있다. 그 외에는 없는 프로젝트와 같은 404다. CSRF 토큰을 검사한다.

**부작용:** 그 프로젝트가 수집해 둔 자료를 모두 지운다 — 게시본·문서·첨부·작업·Issue 사본과 수집 작업·이력이다. 전부 GitHub 원문에서 다시 만들 수 있는 파생 자료이고, 끊었는데 문서 본문이 서버에 남아 있으면 끊었다는 말과 실제가 다르다.

진행 중이거나 예정된 수집이 있으면 취소한다. 첨부 내용은 같은 바이트를 다른 프로젝트가 가리킬 수 있으므로 가리키는 첨부가 하나도 남지 않은 것만 지운다.

**GitHub에는 아무것도 쓰지 않는다.** 끊는다는 것은 서비스가 그 저장소를 더 이상 읽지 않는다는 뜻이지 저장소를 어떻게 한다는 뜻이 아니다. 설치·초대·사용자는 건드리지 않는다.

**출력:** 204다. 끊은 뒤 그 프로젝트 주소는 없는 프로젝트와 같은 404로 답한다.

**재시도:** 이미 끊긴 프로젝트를 다시 부르면 404다. 같은 요청을 두 번 보내도 더 지울 것이 없다.
...
- 링크는 파싱한 뒤 같은 snapshot의 문서·첨부 대상으로 변환한다. 서비스 안을 가리키는 URL은 서비스가 만든 경로만 쓴다.
- 저장소 안이지만 **수집 대상 밖**인 경로(`docs/` 밖의 `rules/`·`AGENTS.md` 등)는 수집 시점 revision에 고정한 GitHub 주소로 바꾸고 `kind`를 `source`로 둔다. revision을 고정하므로 그 게시본이 가리키던 내용이 나중에 바뀌어도 링크가 다른 것을 가리키지 않는다. 그 경로가 그 revision에 실제로 있는지는 확인하지 않는다 — 확인하려면 링크마다 저장소를 한 번씩 더 읽어야 하고, 없으면 GitHub이 404로 답해 읽는 사람이 바로 안다.
- 저장소 밖으로 나가는 경로와 허용하지 않는 규약(`javascript:` 등)은 그대로 열지 않는다. **열 수 없는 링크는 링크로 내보내지 않는다.** 빈 `href`를 남기면 눌렀을 때 현재 문서가 다시 열려, 링크처럼 보이는데 아무 데도 가지 않는다.
