...
| 문서 파일 읽기 | REQ-004, REQ-006 | `GET /repos/{o}/{r}/git/trees/{sha}`, `GET /repos/{o}/{r}/contents/{path}` | Contents: read | 미확인 |
...
- 설치 토큰이 필요한 지점. 지금까지는 사용자 토큰만으로 충분했다. 사용자가 접속하지 않는 동안 수집하려면(TASK-005) 설치 토큰이 필요하다
- 사내 서버에서 webhook 수신이 불가능할 때 주기 조회만으로 REQ-006 인수 기준을 만족하는지

## 다음

1. TASK-005에서 수집을 만들며 문서 파일 읽기와 webhook 행을 확인한다. 사용자가 없을 때 수집하려면 설치 토큰이 필요한지도 그때 판단한다.
