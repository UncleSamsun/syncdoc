...
[API 계약](api-spec.md)의 계약 일람에 `GET /projects/{id}/spec-checklist`를 추가한다. 2026-09-22에 **API-025**로 확정했다. 이 문서를 쓸 때 비어 있던 `API-024`는 그 사이에 브랜치 목록 계약이 가져갔다. 연결 요구는 REQ-008이다. 입력은 `snapshotId` 선택이고, 응답은 아래 형태다.
...
2026-09-22 사용자 확정으로 **별도 화면 UI-013**이 됐다. 이 제안이 적은 "현황 안의 구획"은 채택하지 않았다. 수집 상태·진행률과 규약 준수는 성격이 다른 정보이고 현황 화면이 이미 길다. 아래 규칙은 [화면 명세](../02-ui-spec/ui-screens.md) UI-013으로 옮겼다.
...
- ~~**API-024 응답의 크기 제한.**~~ 2026-09-22 확정. 종류당 50건·전체 500건까지 담고 넘으면 자른 뒤 `truncated`를 참으로 둔다. cursor는 두지 않는다. 자세한 것은 API-025에 있다.
