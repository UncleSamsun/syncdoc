...
**근거:** REQ-001·REQ-002·REQ-006. [화면 명세](../02-ui-spec/ui-screens.md) UI-000이 정한 상단바 조작과 갱신 규칙이 구현에 없다. 2026-09-22 전체 점검에서 확인했다. 상단바에서 누를 수 있는 것이 제품 표식 하나뿐이라 **로그인한 사용자가 로그아웃할 방법이 없다.** 브랜치 전환 절 전체와 20초 주기 갱신·새 버전 배너도 없다. 경로 표의 `/projects/:projectId/documents`는 프로젝트 홈으로 떨어진다.
...
**검증:** 로그아웃을 누르면 세션이 실제로 무효화되고 로그인 화면으로 간다 — 그 뒤 계약 호출이 401이다. 미초대 화면은 세션을 받지 않으므로(허용 목록에 없으면 세션을 주지 않는다) 그 화면의 로그아웃은 이동만으로 충분하다. 브랜치 스위처는 읽을 수 있는 브랜치만 나열하고 직접 입력을 받지 않으며, 바꾸면 `expectedVersion`과 CSRF를 함께 보내고 새 수집이 끝날 때까지 이전 게시본을 보여준다. 권한이 없으면 목록을 열지 않는다. 새 게시본이 게시되면 현황·목록은 조용히 갱신되고 문서 본문에서는 배너만 뜬다 — 읽던 위치를 잃지 않는다.
...
**근거:** REQ-002. 2026-09-23에 요구로 더했다. 지금은 한 번 연결하면 화면에서도 계약으로도 끊을 수 없고, 잘못 연결한 저장소를 지우려면 DB 행을 직접 지워야 한다.

**선행:** TASK-012. 끊는 자리는 연결 설정 화면이다.

**산출물:** [API 계약](../03-tech-spec/api-spec.md)의 새 계약, [데이터 설계](../03-tech-spec/data-model.md)의 삭제 순서, [화면 명세](../02-ui-spec/ui-screens.md) UI-015의 해제 구획, backend/project의 삭제, frontend의 확인 절차.

**검증:** 연결자와 서비스 관리자만 끊을 수 있고 그 외에는 끊는 수단이 보이지 않는다. 저장소 전체이름이 정확히 일치할 때만 실행된다. 진행 중 수집이 있으면 취소하고 끊는다. 끊은 뒤 그 프로젝트 주소는 없는 프로젝트와 같은 404이고, 게시본·문서·첨부·작업·Issue 사본이 데이터베이스에 남지 않는다. GitHub에는 아무것도 쓰지 않는다.

**완료:** 잘못 연결한 저장소를 화면에서 정리할 수 있다. 끊은 뒤 남는 자료가 없다.
